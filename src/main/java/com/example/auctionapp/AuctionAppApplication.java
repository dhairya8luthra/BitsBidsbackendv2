...
public class AuctionAppApplication {
    ...
    // @Bean
    // public CommandLineRunner createAuction(AuctionRepository auctionRepository,
    // UserRepository userRepository) {
    // return (args) -> {
    // User user = userRepository.findByUsername("test");
    // Auction auction = new Auction("Some furniture", "Vintage furniture for
    // sale");
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        // Also weird thing: All browsers I tested actually store the username password
        // after `WWW-Authenticate` challenge,
        // so it caused some trouble figuring out how the client is sending the
        // `Authorization` header even though the
        // frontend code does not explicitly set it.
        http.httpBasic().disable();

        // Thoughts:
        // For form based login, you don't need CSRF protection.
        // It doesn't trigger any side effect other than logging in the user and the
        // attacker won't have the username or password
        // If user is not logged in, that means the user does not have the cookie. So
        // attacker isn't doing anything.
        // If user is logged in, what is the attacker trying to do with login form?
        ...
        // TODO: Figure out if CSRF Token is still needed for REST API using Cookie
        // Auth, this is not clear yet

        http.csrf().csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse());
        // Wasted some time here because POSTMAN decides to re-direct automatically with
        // the same method, not GET
    ...
    public ModelMapper modelMapper() {
        ...
        // ctx.getSource().atZone(ZoneId.of("UTC"));
        ...
        // modelMapper.typeMap(ChatMessage.class,
        // ChatMessageDTO.class).addMappings(mapper -> {
    ...
    // @Bean
    // public RememberMeServices rememberMeServices(final UserDetailsService
    // userDetailsService) {
    // final String SECRET_KEY = "uUx}k@cHAd;=}q@.)@Q6";
    // final PersistentTokenRepository persistentTokenRepository = new
    // JdbcTokenRepositoryImpl();
    // final PersistentTokenBasedRememberMeServices
    // persistentTokenBasedRememberMeServices =
    // new PersistentTokenBasedRememberMeServices(SECRET_KEY, userDetailsService,
